...
// Helper cuntion to maintain min heap property after insertion
...
void insert(PriorityQueue *pq, int value, int priority) {
    ...
    // Maintain heap property by moving the new item up
    heapifyUp(pq, i);
...
void heapifyDown(PriorityQueue *pq, int i) {
    ...
    //If the left child exists and its priority is smaller thatn current smallest
